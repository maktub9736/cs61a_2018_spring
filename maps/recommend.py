...
def group_by_centroid(restaurants, centroids):
    ...
    for restaurant in restaurants:
        key = find_closest(restaurant_location(restaurant), centroids)
        keys.append([key, restaurant])
    return group_by_first(keys)
    # END Question 4
...
def find_predictor(user, restaurants, feature_fn):
    ...
    x_norms = [x - x_mean for x in xs]
    y_norms = [y - y_mean for y in ys]

    S_xx = sum([x_norm ** 2 for x_norm in x_norms])
    S_yy = sum([y_norm ** 2 for y_norm in y_norms])
    S_xy = sum([x_norms[i] * y_norms[i] for i in range(len(x_norms))])
...
def search(query, restaurants):
    ...
    sel = []
    for restaurant in restaurants:
        for strs in restaurant_categories(restaurant):
            if query in strs:
                sel.append(restaurant)
    return sel
