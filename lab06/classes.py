...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        copy_pd = player.deck.copy()
        for i in range(len(copy_pd.cards)):
            player.deck.cards[i].attack += other_card.attack
            player.deck.cards[i].defense += other_card.defense

        opponent.deck.cards = [opponent.deck.cards[k]\
            for k in range(orig_opponent_deck_length)\
            if opponent.deck.cards[k].attack != other_card.attack\
            and opponent.deck.cards[k].defense != other_card.defense]
