...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            "*** YOUR CODE HERE ***"
            if isinstance(insect, QueenAnt) and not insect.initiated:
                return
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        proceed = self.min_range
        while proceed < self.max_range:
            if current_place.bees or current_place.entrance == hive:
                break
            else:
                current_place = current_place.entrance
                proceed += 1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        self.armor -= amount
        hold_bees = self.place.bees[:]
        if self.armor <= 0:
            self.place.remove_insect(self)
            for bee in hold_bees:
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        if self.place.bees and self.place == bee.place:
    ...
    def action(self, colony):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        #sth to eat, not digesting
        if self.digesting == 0 or self.time_to_digest == 0:
            if self.place.bees:
                self.eat_bee(random_or_none(self.place.bees))
                self.digesting = 1
            else:
                self.timer = self.time_to_digest
        else:
            self.timer -= 1
            if self.timer == 0:
                self.digesting = 0
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if self.contained_ant or (other.is_ant and other.is_container):
            return False
        else:
            return True
        # END Problem 9
            ...
            while True:
                ...
                if ant:
                    if not ant.buffed:
                        ...
                        ant.buffed = True
                    if hasattr(ant, 'contained_ant') and ant.contained_ant and ant.contained_ant.buffed == False:
                        ant.contained_ant.damage = 2 * ant.contained_ant.damage
                        ant.contained_ant.buffed = True
...
def make_slow(action, bee):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    if not colony.time % 2 and not bee.effected:
        bee.effected = True
        return action
