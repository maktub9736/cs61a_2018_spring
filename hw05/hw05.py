...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    left_torq = length(left(m)) * total_weight(end(left(m)))
    right_torq = length(right(m)) * total_weight(end(right(m)))
    x = (left_torq == right_torq)
    if is_weight(end(left(m))) and is_weight(end(right(m))):
        return x
    elif is_mobile(end(left(m))) and is_weight(end(right(m))):
        return x and balanced(end(left(m)))
    elif is_weight(end(left(m))) and is_mobile(end(right(m))):
        return x and balanced(end(right(m)))
    else:
        return x and balanced(end(left(m))) and balanced(end(right(m)))
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, pw):
        nonlocal balance, password, wrong_pw
        if len(wrong_pw) >= 3:
            return "Your account is locked. Attempts: %s"%(wrong_pw)
        elif pw != password:
            wrong_pw.append(pw)
...
def generate_paths(t, x):
    ...
    def find_path(t, x):
        if is_leaf(t):
            ...
            path = []
            for b in branches(t):
                subpath = find_path(b, x) # --> a list of lists(paths) starting from b
                for sp in subpath:
                    sp = [label(t)] + sp
                    path.append(sp)
            return path                  # --> return a list of paths starting from t
    all_paths = find_path(t, x)
    sel_paths, selected = [], []
    for path in all_paths:               # [0, 2, 1, 2]
        if x in path:
            sel_paths.append(path)       # sel_paths: from all_paths choose those contain x
    for path in sel_paths:               # from sel_paths find paths and subpaths
        while len(path) > 0:
            if path[-1] == x and path not in selected:
...
def check_par():
    ...
    True
    """
    r1 = interval(8, 10) # Replace this line!
...
def quadratic(x, a, b, c):
    ...
    def value(x):
        return a * (x ** 2) + b * x + c
    v1 = value(lower_bound(x))
    v2 = value(upper_bound(x))
    v3 = value(steady_point)
    if steady_point < lower_bound(x) or steady_point > upper_bound(x):
